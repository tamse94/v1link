...
export default function LoginModal({ onLoginSuccess }) {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      // Langsung narik email & password dari tabel settings di Supabase
      ...
      if (dbError || !data) {
        setError('Gagal terhubung ke database pengaturan.');
        setLoading(false);
        return;
      }

      // Cocokin inputan lu sama data di database
    ...
    <div className="fixed inset-0 z-[999] flex items-center justify-center p-4 overflow-hidden">
      ...
      <div className="relative w-full max-w-sm animate-[fadeIn_0.3s_ease-out]">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          >
            {loading ? <span className="material-symbols-outlined animate-spin">progress_activity</span> : <>Sign In <span className="material-symbols-outlined text-sm">arrow_forward</span></>}
          </button>
        </form>
      </div>
    </div>
